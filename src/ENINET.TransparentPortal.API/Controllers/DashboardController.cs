...
namespace ENINET.TransparentPortal.API.Controllers
{
    ...
    public class DashboardController : ControllerBase
    {
        ...
        [HttpGet("reportformonths/{acronym}/{year}")]

        [ProducesResponseType(typeof(ApiResult<IList<ElementStatisticDto>>), 200)]
        public async Task<ApiResult<IList<ElementStatisticDto>>> GetYearReport(string acronym, int year)
        {
            ...
            var elements = await _repository.ElementSite.FindByCondition(null, r => r.Acronym.ToLower() == acronym.ToLower() && authorizedSites.Contains(r.Acronym), false).ToListAsync();
